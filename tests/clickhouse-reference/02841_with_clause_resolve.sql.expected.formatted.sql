...
-- Distortion
  ...
  (x, amount) -> clamp(if(x > 0, pow(x, amount), negate(pow(negate(x), amount)))) AS power_distortion,
...
-- Delay
  ...
  delay(time, (time -> power_distortion(sine_wave(time * 80 + sine_wave(time * 2)), lfo(0.5, 1, sine_wave, time / 16)) * running_envelope(60, time, 0, 0.0, 0.01, 0.1)), 0.2, 0.5, 5) AS kick
