SELECT lagInFrame(2::UInt128, 2, number) OVER w
FROM numbers(10)
WINDOW w AS (ORDER BY number ASC);

SELECT leadInFrame(2::UInt128, 2, number) OVER w
FROM numbers(10)
WINDOW w AS (ORDER BY number ASC);

SELECT lagInFrame(2::UInt64, 2, number) OVER w
FROM numbers(10)
WINDOW w AS (ORDER BY number ASC);

SELECT leadInFrame(2::UInt64, 2, number) OVER w
FROM numbers(10)
WINDOW w AS (ORDER BY number ASC);
...
WINDOW w AS (PARTITION BY p ORDER BY number ASC NULLS FIRST ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED PRECEDING);