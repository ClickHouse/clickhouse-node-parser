...
SELECT caseWithExpression(materialize(assumeNotNull(materialize(NULL))), materialize(NULL), NULL, NULL); -- { serverError ILLEGAL_TYPE_OF_ARGUMENT }
...
SELECT count()
WHERE caseWithExpression(1, assumeNotNull(materialize(NULL)), 1, 1); -- { serverError ILLEGAL_TYPE_OF_COLUMN_FOR_FILTER }