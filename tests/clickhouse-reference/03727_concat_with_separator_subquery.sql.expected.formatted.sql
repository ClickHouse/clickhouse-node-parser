...
SELECT concatWithSeparator(c0, 1)
FROM (
        SELECT 'a' AS c0
    ) AS tx;
...
SELECT concatWithSeparator(c0, 'b', 1)
FROM (
        SELECT CAST(NULL AS Nullable(String)) AS c0
    ) AS tx;

SELECT concatWithSeparator('a', c1, 'b')
FROM (
        SELECT 1 AS c1
    ) AS tx;
...
SELECT concatWithSeparator('a', c1, 'b')
FROM (
        SELECT CAST(1 AS Nullable(UInt8)) AS c1
    ) AS tx;
...
SELECT concatWithSeparator('-', [1, 2, 3], [4, 5, 6])
FROM numbers(3);

SELECT concatWithSeparator('-', (1, 'a'), (2, 'b'))
FROM numbers(3);
