...
WHERE d NOT IN (-1, 0); -- { serverError ARGUMENT_OUT_OF_BOUND }
...
WHERE d NOT IN (-9223372036854775808, 0); -- { serverError ARGUMENT_OUT_OF_BOUND }

SELECT d
FROM values('d Decimal(9, 8)', 0, 4)
WHERE d NOT IN (18446744073709551615, 0); -- { serverError ARGUMENT_OUT_OF_BOUND }
