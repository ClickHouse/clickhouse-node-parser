...
SELECT arrayFold((acc, x) -> if(x % 2, (arrayPushBack(acc.1, x), acc.2), (acc.1, arrayPushBack(acc.2, x))), [1, 2, 3, 4, 5, 6], (emptyArrayInt64(), emptyArrayInt64()));
...
SELECT arrayFold((acc, x) -> if(x % 2, arrayPushFront(acc, x), arrayPushBack(acc, x)), range(number), emptyArrayUInt64())
FROM `system`.numbers
LIMIT 5;
...
SELECT
    ...
    arrayFold((acc, str) -> position(line, str), splitByNonAlpha(line), 0::UInt64)
FROM tab;
